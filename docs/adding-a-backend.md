...
`src/Backend/ServerBackendInterface.php` — four methods:
...
| `detectModelMetadata($entry)` | Any subset of `cost_input` / `cost_output` (USD per **million** tokens), `quality_tier` (1–5), `context_length` (tokens). Empty array when nothing can be inferred. Values only fill fields that are still unset — re-discovery never clobbers manual edits, so prefilling is always safe. |
