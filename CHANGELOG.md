...
- **`decide()` works with any model**: a decision is a shape of question, not a capability. A decision model answers natively; every other model is prompted for the same answers as JSON and normalized into the same shape (without invented confidence). Smart routes can therefore hold Jev and a chat model as candidates for the same job, and fail over between them; `text_classification` works on both.
