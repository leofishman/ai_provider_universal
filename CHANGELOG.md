...
- **`deepseek` backend**: OpenAI-compatible at a fixed endpoint, with list prices and context prefilled per model. DeepSeek bills on a peak/off-peak schedule, so smart routing applies the off-peak discount (16:30-00:30 UTC) as a price multiplier at decision time; stored costs stay at list price and remain editable.
